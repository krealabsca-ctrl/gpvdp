...
**Corte:** 3 de setiembre de 2026 · migración **0080** · **86 tablas** de negocio (más
`schema_migrations`, que crea golang-migrate) · **937 columnas** · **321 endpoints**
...
| 3º | **`DICCIONARIO-DATOS.md`** | Las 86 tablas con sus 937 columnas: tipo exacto, obligatoriedad, valor por defecto, claves primarias y foráneas | 65 KB |
...
Queda la estructura completa: 86 tablas de negocio, 937 columnas, 286 índices, 250 claves foráneas.
...
hoy la 80). Eso importa: sin esa fila, el backend arranca leyendo versión NULL, cree que la base está
...
tablas con `schema_migrations` en la versión 80 y `dirty = false`.
...
1. **El esquema se cambia con migraciones, no editando `schema.sql`.** La fuente de verdad son los
   80 archivos de `backend/migrations/`, que el backend aplica solo al arrancar. Ese archivo es una
